...
public class HappyWavesJob {
    public static void main(String[] args) throws Exception {
        ...
        // Kafka sink
        KafkaSink<WaveEvent> uniqueWavesSink = KafkaSink.<WaveEvent>builder()
                .setBootstrapServers(config.getProperty("bootstrap.servers"))
                .setKafkaProducerConfig(config)
                .setRecordSerializer(KafkaRecordSerializationSchema.builder()
                        .setTopic("rt.waves.v1")
                        .setValueSerializationSchema(
                                (SerializationSchema<WaveEvent>) waveEvent -> {
                                    try {
                                        // Convert WaveEvent to JSON bytes
                                        return objectMapper.writeValueAsBytes(waveEvent);
                                    } catch (Exception e) {
                                        throw new RuntimeException("Failed to serialize WaveEvent to JSON", e);
                                    }
                                }
                        )
                        .build()
        ...
        personListStream
                ...
                .flatMap(new StatefulWaveEmitter())
                .sinkTo(uniqueWavesSink);


        env.execute("Waves Events Job");
    }

    // Load properties from resources
    public static Properties readConfig(final String configFile) throws IOException {
        final Properties config = new Properties();
        try (InputStream inputStream = HappyWavesJob.class.getClassLoader().getResourceAsStream(configFile)) {
            if (inputStream == null) {
